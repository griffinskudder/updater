...
//
// The raw key value is never persisted; only the SHA-256 hex digest (KeyHash)
// and an 8-character display prefix (Prefix) are stored. The raw key is
// returned exactly once — at creation time — and cannot be recovered afterwards.
//
// Keys are generated by GenerateAPIKey and follow the format upd_<44 base64url chars>.
// Auth middleware resolves incoming Bearer tokens via GetAPIKeyByHash.
type APIKey struct {
